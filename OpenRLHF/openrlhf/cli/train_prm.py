...
def train(args):
    ...
    # 准备多个评估数据集
    eval_dataloaders = {}
    if getattr(args, "eval_dataset", None) and getattr(args, "eval_splits", None):
        for split_name in args.eval_splits:
            ...
            eval_dataset = ProcessRewardDataset(eval_data, tokenizer, tokenizer.model_max_length, strategy)
            ...
            eval_dataloaders[split_name] = eval_dataloader
            strategy.print(f"Loaded eval dataset: {args.eval_dataset} split: {split_name}, size: {len(eval_dataset)}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--eval_splits", type=str, nargs="*", default=None,
                        help="Eval dataset splits (e.g., gsm8k math olympiadbench omnimath)")
