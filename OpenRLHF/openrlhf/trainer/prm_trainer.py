...
class ProcessRewardModelTrainer(ABC):
    ...
    Args:
        ...
        eval_dataloaders (dict): Dictionary of dataloaders for evaluation datasets.
                                 Key is the dataset name, value is the dataloader.
    ...
    def __init__(
        ...
        self.eval_dataloaders = eval_dataloaders if eval_dataloaders else {}
    ...
    def save_logs_and_checkpoints(self, args, global_step, step_bar, logs_dict={}, client_states={}):
        ...
        # eval
        if self.eval_dataloaders and global_step % args.eval_steps == 0:
            self.evaluate(self.eval_dataloaders, global_step)
    ...
    def evaluate(self, eval_dataloaders, steps=0):
        self.model.eval()
        with torch.no_grad():
            for dataset_name, eval_dataloader in eval_dataloaders.items():
                ...
                step_bar = tqdm(
                    ...
                    desc=f"Eval [{dataset_name}] at step {steps}",
                    disable=not self.strategy.is_rank_0(),
                )
                ...
                for data in eval_dataloader:
                    ...
                    loss_sum += loss.item()
                    acc_sum += acc.item()
                    bar_dict = {f"{dataset_name}_loss": loss_sum / times, f"{dataset_name}_acc": acc_sum / times}
                    step_bar.update()
                    logs = self.strategy.all_reduce(bar_dict)
                    step_bar.set_postfix(logs)

                # 记录每个数据集的最终结果
                avg_loss = loss_sum / times
                avg_acc = acc_sum / times

                if self._wandb is not None and self.strategy.is_rank_0():
                    self._wandb.log({
                        f"eval/{dataset_name}/loss": avg_loss,
                        f"eval/{dataset_name}/acc": avg_acc,
                        "eval/global_step": steps,
                    })
                elif self._tensorboard is not None and self.strategy.is_rank_0():
                    self._tensorboard.add_scalar(f"eval/{dataset_name}/loss", avg_loss, steps)
                    self._tensorboard.add_scalar(f"eval/{dataset_name}/acc", avg_acc, steps)
