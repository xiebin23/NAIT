...
class PRMLoss(nn.Module):
    ...
    def __init__(self, placeholder_token_id: int, reward_token_ids: Optional[list[int]] = None):
        super().__init__()
        self.IGNORE_INDEX = -100
        # 正样本的权重是 1/6, 负样本的权重是 5/6, tensor device will be automatically inferred during training
        self.register_buffer("loss_weight", torch.tensor([1/6, 5/6], dtype=torch.float32))
        # self.loss = nn.CrossEntropyLoss(ignore_index=self.IGNORE_INDEX, weight=self.loss_weight)
    ...
    def forward(self, inputs: torch.Tensor, logits: torch.Tensor, labels: torch.Tensor, *, return_acc: bool = False):
        ...
        weight = self.loss_weight.to(logits.device)
